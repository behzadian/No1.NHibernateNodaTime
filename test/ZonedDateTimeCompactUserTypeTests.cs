...
/// </summary>
public class ZonedDateTimeCompactUserTypeTests(NHibernateCompositeTestFixture fixture) : IClassFixture<NHibernateCompositeTestFixture>
{
	...
	public async Task ShouldPersistZonedDateTimeIn5Columns() {
		...
		var entity = new ZonedDateTimeCompactEntity() { Valauable = zdt };
		...
		using (var session = _sessionFactory.OpenSession()) {
			var sql = @"
				SELECT Valauable_Zone_ID, Valauable_UTC
				FROM ""zoned_date_time_compacts""
				WHERE id = :id";
		...
		ZonedDateTimeCompactEntity? retrievedEvent;
		using (var session = _sessionFactory.OpenSession()) {
			retrievedEvent = await session.GetAsync<ZonedDateTimeCompactEntity>(savedId);
		}
	...
	[Fact]
	public async Task ShouldPreserveSecondsPrecision() {
		var zdt = Instant
		...
		// Assert
		retrievedEvent.Should().NotBeNull();
		retrievedEvent.Valauable.Should().NotBe(zdt);
		retrievedEvent.Valauable.ToInstant().ToUnixTimeSeconds().Should().Be(zdt.ToInstant().ToUnixTimeSeconds());
		retrievedEvent.Valauable.ToInstant().OnlyNanoseconds().Should().Be(0);
	}
	...
	public async Task ShouldHandleNullable() {
		...
		var entity = new ZonedDateTimeCompactEntity() { Valauable = now, Nullable = null };
		...
		using (var session = _sessionFactory.OpenSession()) {
			var sql = @"
				SELECT Nullable_Zone_ID, Nullable_UTC
				FROM ""zoned_date_time_compacts""
				WHERE id = :id";
