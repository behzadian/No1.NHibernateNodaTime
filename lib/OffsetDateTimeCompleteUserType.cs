...
public sealed class OffsetDateTimeCompleteUserType : ICompositeUserType
{
	internal static readonly string[] Columns = [.. LocalDateTimeCompleteUserType.Columns, .. OffsetUserType.Columns];

	private static readonly int DateTimeColumnsCount = LocalDateTimeCompleteUserType.Columns.Length;
	...
	[
		..LocalDateTimeCompleteUserType.Instance.PropertyTypes,
	...
	object? ICompositeUserType.NullSafeGet(DbDataReader dr, string[] names, ISessionImplementor session, object owner) {
		...
		var date = (LocalDateTime?)LocalDateTimeCompleteUserType.Instance.NullSafeGet(dr, dateNames, session, owner);
	...
	void ICompositeUserType.NullSafeSet(DbCommand cmd, object? value, int index, bool[] settable, ISessionImplementor session) {
		if (value is OffsetDateTime val) {
			LocalDateTimeCompleteUserType.Instance.NullSafeSet(cmd, val.LocalDateTime, index, settable, session);
			OffsetUserType.Instance.NullSafeSet(cmd, val.Offset, index + DateTimeColumnsCount, session);
		} else {
			LocalDateTimeCompleteUserType.Instance.NullSafeSet(cmd, null, index, settable, session);
	...
	object? ICompositeUserType.GetPropertyValue(object component, int property) {
		if (component is OffsetDateTime val) {
			return property < DateTimeColumnsCount ? LocalDateTimeCompleteUserType.Instance.GetPropertyValue(val.LocalDateTime, property) : val.Offset;
		} else {
